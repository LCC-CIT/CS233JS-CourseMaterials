...
> **Note:** The linked MDN tutorials and GitHub code examples use the older `.then()` promise chaining syntax. The notes below demonstrate the modern `async`/`await` pattern, which is now the standard for writing asynchronous JavaScript.
...
Here are the updated lecture notes, revised to focus on the modern `async`/`await` pattern while preserving the structure of your original document.
