...
### Classes that Contain Instances (Objects) of Other Classes
...
- When a class contains an instance of another class, this is called *aggregation*, this is sometimes called the "has a" relationship.
  ...
  - There is another closely related relationship called *composition* which is sometimes called the "whole-part" relationship.

- Aggregation, and also composition, are the ways to build an application using multiple objects where each object represents a "thing" in the application.
...
This application has already been separated into modules and already has objects (old-style, not using classes). These are the modules and the classes we will put in them:

- Index.js (main)  
  Controls the overall flow of application execution and handles events.  
  No classes.

- gameLogic.js

  - `Card` class  
    Has  properties holding letters or numbers that represent suit and value.
